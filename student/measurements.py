...
class Sensor:
    ...
    def in_fov(self, x):
        ...
        # get current object position in homogeneous coordinates
        pos_vehicle = np.ones((4,1))
        pos_vehicle[0:3] = x[0:3]

        # transform position to sensor coordinates
        pos_sensor = self.veh_to_sens * pos_vehicle
        visible_flag = False

        # exclude negative x-axis
        if pos_sensor[0] > 0:
    ...
    def get_hx(self, x):    
        ...
        if self.name == 'lidar':
            ...
            # exclude negative x axis
            if pos_sensor[0]>0:
                # project position to camera coordinates
...
class Measurement:
    ...
    def __init__(self, num_frame, z, sensor):
        ...
        if sensor.name == 'lidar':
            ...
            # load cconfiguration parameters
